...
const submitButton = 'button[id="submit_form"]'
...
function addRowToItemsTable(recordObject, loadedFromUpdateOperation = false) {
  ...
  var hiddenItemId =
    '<input type="hidden" name="item_id[' + currentRowId + ']" class="form-control" value="' + recordObject.id + '">'
  var inputItemName =
    `<label class="form-label mb-0" role="button" name="name[${currentRowId}]">${recordObject.name}</label> ` +
...
var inputQuantity =
  ...
  var inputDeleteButton =
    '<button type="button" class="btn btn-outline-danger ' + removeClass + '"><i class="bx bx-trash me-0"></i></button>'
...
newRow.append("<td>" + inputPKD + "</td>")
newRow.append("<td>" + inputEXP + "</td>")
...
function returnDecimalValueByName(inputBoxName) {
  ...
  return Number.parseFloat(inputBoxValue)
}
...
function createLabelData() {
  ...
  for (let i = 0; i < rowCount; i++) {
    if (returnDecimalValueByName("quantity[" + i + "]") > 0) {
      ...
      const price = $("#display_mrp_on_label").is(":checked")
        ? $('label[name="mrp[' + i + ']"]').text()
        : $('label[name="sale_price[' + i + ']"]').text()
      const companyName = appCompanyName
const pkdDate = $('input[name="pkd_date[' + i + ']"]').val()
const expDate = $('input[name="exp_date[' + i + ']"]').val()
