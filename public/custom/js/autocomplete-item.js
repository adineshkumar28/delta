...
function addItemToTable(item) {
    ...
    // Blade values injected correctly
    var showMrpClass = "{{ app('company')['show_mrp'] ? '' : 'd-none' }}";
    var showDiscountClass = "{{ app('company')['show_discount'] ? '' : 'd-none' }}";

    var rowHtml = `
        <tr data-row-index="${rowIndex}">
            
            <!-- MRP Column -->
            <td class="${showMrpClass}">
                <input type="text" 
                       name="mrp[]" 
                       class="form-control mrp cu_numeric text-end" 
                       value="${_formatNumber(item.mrp ?? 0)}" 
                       readonly>
            </td>
            ...
            <td>
                <input type="text" 
                       name="quantity[]" 
                       class="form-control quantity cu_numeric text-end" 
                       value="1">
            </td>
            ...
            <td>
                <input type="text" 
                       name="sale_price[]" 
                       class="form-control sale_price cu_numeric text-end" 
                       value="${_formatNumber(item.sale_price ?? 0)}">
            </td>

            <!-- Discount Column -->
            <td class="${showDiscountClass}">
                <input type="text" 
                       name="discount[]" 
                       class="form-control discount cu_numeric text-end" 
                       value="0">
            </td>
        </tr>
    `;
    ...
    // Append row
    $('#invoiceItemsTable tbody').append(rowHtml);
